...
const deployFunction: DeployFunction = async function ({
  ...
  const { address, newlyDeployed } = await deploy("SolidlyPoolFactory", {
    from: deployer,
  ...
  if (!(await masterDeployer.whitelistedFactories(address))) {
    console.debug("Add SolidlyPoolFactory to MasterDeployer whitelist");
...
deployFunction.tags = ["SolidlyPoolFactory"];
